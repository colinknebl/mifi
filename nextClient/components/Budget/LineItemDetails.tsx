...
class LineItemDetails extends React.Component<any, any> {
	...
	public render() {
		const state = this.props.appData.state,
			budgetGroupListPosition =
				state.app.budget.budgetPlus.budgetGroupListPosition,
			lineItemListPosition =
				state.app.budget.budgetPlus.budgetGroupLineItemListPosition,
			lineItemToRender =
				state.finances.budget.budgetGroups[budgetGroupListPosition]
					.lineItems[lineItemListPosition],
			budgetGroupHeader = state.app.budget.budgetPlus.budgetGroupHeader,
			detailsHeader =
				budgetGroupHeader.charAt(0).toUpperCase() +
				budgetGroupHeader.slice(1).toLowerCase(),
			...
			actualAmount = methods.financial.formatAmount(
				lineItemToRender.actual
			),
		...
		this.progressBarPercent =
			lineItemToRender.actual / lineItemToRender.planned || 0;
		...
		return (
			<aside className="LineItemDetails">
				...
				<section className="LineItemDetails__metadata">
					...
					<p
						className="metadata__line-item-title"
						title={lineItemToRender.title}
					>
						{lineItemToRender.title}
					</p>
					<p className="metadata__amount">{amount}</p>
					<span className="metadata__progress-bar">
						<span
							id={this.progressBarId}
							className="metadata__progress-bar--inner"
						/>
					</span>
					<p className="metadata__spent">
						<span className="metadata__spent--green">
							{actualAmount}
						</span>{' '}
						{spent}
					</p>
				...
				<section className="LineItemDetails__details">
					{detailsHeader !== 'Income' && !lineItemToRender.isFund ? (
						<div className="details__fund">
							...
							<i
								className={`fa fa-caret-${caretDirection} details__icon`}
								onClick={this.fundCaretClicked}
							/>
							<div id={this.fundMoreId} className="fund__more">
								<p>
									Funds carry balances month to month, letting
									you save toward a goal over time.
								</p>
								<button
									className="btn btn-blue fund__more__btn"
									onClick={this.setLineItemAsFund}
								>
									Make this a Fund
								</button>
							</div>
