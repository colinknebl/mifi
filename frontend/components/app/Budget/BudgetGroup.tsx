...
import BudgetGroupLineItem from './BudgetGroupLineItem';
...
class BudgetGroup extends React.Component {
	...
	public render() {
		...
		const plannedSum = formatMoney(sum(this.props.lineItems, 'planned')),
			receivedSum = formatMoney(sum(this.props.lineItems, 'actual')),
			icon = this.state.minimized ? 'down' : 'up';
		let className =
			header === 'drop target'
				? 'BudgetGroup BudgetGroup--drop-target'
				: 'BudgetGroup';
		...
		return (
			...
			>
				...
				<h6 className="BudgetGroup__header font--dark">
					{header.toUpperCase()}
					<button
						className="btn-caret BudgetGroup__caret-icon"
						onClick={this.budgetGroupExpandCloseHandler}
					>
						<i
							className={`fa fa-caret-${icon}`}
							aria-hidden="true"
						/>
					</button>
				</h6>
				<span className="BudgetGroup__column--right column-header__planned font--light">
					Planned
				</span>
				...
				{!this.state.minimized ? (
					<ShowLineItems
						{...{
							...
							plannedSum,
							receivedSum,
							budgetGroupListPosition: this.props
	...
	return (
		<>
			<ul className="BudgetGroup__line-item-list">
				{props.lineItems.map(
					(lineItem: IBudgetGroupLineItem, num: number) => {
						return (
							<BudgetGroupLineItem
								key={num}
			...
			{props.calcTotals ? (
				<Totals
					plannedSum={props.plannedSum}
					receivedSum={props.receivedSum}
...
function Totals(props: any): any {
	return (
		<>
			...
			<span className="BudgetGroup__column--right">
				{props.plannedSum}
			</span>
			<span className="BudgetGroup__column--right">
				{props.receivedSum}
...
function AddItemBtn(props: any) {
	return (
		<button className="BudgetGroup__add-item-btn" onClick={props.onClick}>
			+ Add Item
		</button>
	);
}
